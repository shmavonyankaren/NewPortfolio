...
export function Globe({ globeConfig, data }: WorldProps) {
  ...
  // Initialize globe and update whenever config or data change
  useEffect(() => {
    ...
    if (!globeRef.current) {
      ...
      const hasGlobe = groupRef.current.children.includes(globeRef.current);
      if (!hasGlobe) {
        groupRef.current.add(globeRef.current);
      }
    }
    ...
    globe
      ...
      .arcColor(
        ((e: unknown) => (e as Position).color) as unknown as (
          t: number
        ) => string
      )
